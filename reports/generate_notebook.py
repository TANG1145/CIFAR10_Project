#!/usr/bin/env python3
"""Generate the CIFAR-10 report notebook."""
...
# ============ Title ============
add_markdown("""# CIFAR-10 图像分类实验报告
...
add_markdown("""**分析**: 从图中可见，10 个类别的样本数量完全一致，不存在类别不平衡问题，因此无需针对类别不平衡采取特殊处理策略（如加权损失或重采样）。
...
add_code("""display(Image(filename='../results/final/sample_images.png'))""")

add_markdown("""**观察**: CIFAR-10 图像尺寸仅为 32x32，细节较为模糊，部分类别（如 cat 与 dog、automobile 与 truck）在视觉上区分度较低，这对分类模型提出了较高挑战。
...
add_code("""display(Image(filename='../results/final/pixel_statistics.png'))""")

add_markdown("""**分析**: 三个颜色通道的像素值分布接近，均值和标准差差异不大。基于此，后续预处理采用统一的标准化参数。
...
add_code("""display(Image(filename='../results/final/augmentation_comparison.png'))""")

add_markdown("""**说明**: 数据增强在不增加实际样本数量的前提下，通过几何变换有效扩充了训练数据的多样性，是抑制过拟合的重要手段。
...
**分析**:
- MLP 仅达到 39.31% 的准确率，远低于随机猜测的 10%，说明全连接网络难以捕捉图像的空间特征。
...
add_code("""display(Image(filename='../results/final/model_comparison_curves.png'))""")
...
add_code("""display(Image(filename='../results/final/accuracy_bar_chart.png'))""")

add_markdown("""**分析**: 从柱状图可以清晰看出三种模型之间的性能鸿沟。ResNet-18 相比 CNN 提升了约 11.5 个百分点，相比 MLP 提升了约 55.8 个百分点。
...
add_code("""display(Image(filename='../results/final/per_class_comparison.png'))""")
...
# ============ Part 5 ============
add_markdown("""## 五、结论与可能改进

### 5.1 关键结论

通过本次实验，我们得出以下核心结论：

1. **模型架构决定性能上限**: 在 CIFAR-10 数据集上，ResNet-18 (95.16%) 显著优于 CNN (83.66%) 和 MLP (39.31%)。残差连接和深层特征提取能力是小图像分类任务取得高性能的关键。
...
with open('../reports/report.ipynb', 'w', encoding='utf-8') as f:
    json.dump(notebook, f, ensure_ascii=False, indent=2)

print("Notebook saved to ../reports/report.ipynb")
